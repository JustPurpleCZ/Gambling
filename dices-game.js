...
// Resize game container to maintain 2:1 ratio
...
function renderDiePosition(die) {
    ...
    const pixelX = canvasOffsetX + (die.xPercent * canvas.clientWidth) / 100;
    const pixelY = canvasOffsetY + (die.yPercent * canvas.clientHeight) / 100;
...
// State variables for auto-roll logic
...
function updateCupPosition() {
    ...
    const xPx = canvasOffsetX + (cupXPercent * canvas.clientWidth) / 100;
    const yPx = canvasOffsetY + (cupYPercent * canvas.clientHeight) / 100;
...
document.addEventListener('mousemove', (e) => {
    ...
    cupXPercent = (relX / canvas.clientWidth) * 100;
    cupYPercent = (relY / canvas.clientHeight) * 100;
    updateCupPosition();
...
function collectDice() {
    ...
    const cupWidth = containerWidth * 0.10;
    const cupHeight = containerHeight * 0.50;
    const diceSize = containerWidth * 0.08;
    ...
    const cupPxX = canvasOffsetX + (cupXPercent * canvas.clientWidth) / 100;
    const cupPxY = canvasOffsetY + (cupYPercent * canvas.clientHeight) / 100;

    const cupCenterX = cupPxX + cupWidth * 0.6;
    const cupCenterY = cupPxY + cupHeight * 0.85;
    const collectRadius = containerHeight * 0.1;
    ...
    for (let i = dice.length - 1; i >= 0; i--) {
        ...
        const diePxX = canvasOffsetX + (die.xPercent * canvas.clientWidth) / 100;
        const diePxY = canvasOffsetY + (die.yPercent * canvas.clientHeight) / 100;
...
async function lockDie(die) {
    ...
    const targetY = (containerH * 0.17) + (lockedDice.length * (containerH * 0.117));
...
async function unlockDie(die) {
    ...
    const targetX = canvasOffsetX + (randomXPercent * canvas.clientWidth) / 100;
    const targetY = canvasOffsetY + (randomYPercent * canvas.clientHeight) / 100;
...
function collectAllDiceIntoCup() {
    ...
    const collectTargetX = canvasOffsetX + (85 * canvas.clientWidth) / 100;
    const collectTargetY = canvasOffsetY + (120 * canvas.clientHeight) / 100;
...
function update() {
    ...
    const container = document.getElementById('game-container');
    const diceSize = container.clientWidth * 0.08;

    dice.forEach(die => {
        if (die.rolling) {
            die.rollTime += 16;

            let diePxX = (die.xPercent * canvas.clientWidth) / 100;
            let diePxY = (die.yPercent * canvas.clientHeight) / 100;
            ...
            if (diePxX > canvas.clientWidth - diceSize) {
                diePxX = canvas.clientWidth - diceSize;
                die.vx *= -0.6;
            }
            if (diePxX < 0) {
                diePxX = 0;
            ...
            if (diePxY > canvas.clientHeight - diceSize) {
                diePxY = canvas.clientHeight - diceSize;
                die.vy *= -0.6;
                die.vx *= 0.9;
            }
            if (diePxY < 0) {
            ...
            die.xPercent = (diePxX / canvas.clientWidth) * 100;
            die.yPercent = (diePxY / canvas.clientHeight) * 100;

            dice.forEach(other => {
                if (die === other) return;
                const otherPxX = (other.xPercent * canvas.clientWidth) / 100;
                const otherPxY = (other.yPercent * canvas.clientHeight) / 100;
                ...
                if (dist < diceSize && dist > 0) {
                    ...
                    other.xPercent = ((otherPxX + nx * overlap * 0.5) / canvas.clientWidth) * 100;
                    other.yPercent = ((otherPxY + ny * overlap * 0.5) / canvas.clientHeight) * 100;
                    die.xPercent = (diePxX / canvas.clientWidth) * 100;
                    die.yPercent = (diePxY / canvas.clientHeight) * 100;
